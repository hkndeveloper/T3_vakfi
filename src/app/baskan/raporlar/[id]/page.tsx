...
async function updateReportAction(formData: FormData) {
  ...
  const participantCount = parseInt(String(formData.get("participantCount") ?? "0"));
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000 font-outfit">
      ...
      <div className="flex items-center gap-4 group">
        <Link 
          ...
          className="h-12 w-12 rounded-2xl bg-white shadow-xl shadow-slate-200/50 flex items-center justify-center text-corporate-blue hover:bg-corporate-blue hover:text-white transition-all active:scale-95 border border-slate-100"
        >
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <p className="t3-label">RAPOR YÖNETİMİ / {report.reportType}</p>
          <h2 className="text-xl font-black text-slate-950 uppercase tracking-tight">{report.title}</h2>
        </div>
      </div>

      {/* Hero Stats Section */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="col-span-1 md:col-span-2 relative overflow-hidden rounded-[3rem] bg-slate-950 p-10 md:p-14 text-white shadow-2xl group border border-white/5">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full bg-corporate-blue/20 border border-corporate-blue/30 px-4 py-1.5 text-[10px] font-black text-white uppercase tracking-[0.2em] mb-8 animate-pulse">
              <ClipboardCheck className="h-3.5 w-3.5" /> RAPOR GENEL BAKIŞ
            </div>
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter leading-tight uppercase mb-8 italic">
              {report.title}
            </h1>
          ...
          <div className="absolute -right-20 -top-20 h-[500px] w-[500px] rounded-full bg-corporate-blue/10 opacity-30 blur-[130px] pointer-events-none" />
        ...
        <div className="rounded-[3rem] bg-white border border-slate-100 p-8 shadow-2xl shadow-slate-200/50 flex flex-col justify-center relative overflow-hidden group">
           <div className="relative z-10">
              <div className="h-14 w-14 rounded-2xl bg-orange-50 flex items-center justify-center text-corporate-orange mb-6 border border-orange-100 shadow-sm">
                 <Users className="h-7 w-7" />
              </div>
              <p className="t3-label mb-1">Katılımcı Sayısı</p>
              <h3 className="text-4xl font-black text-slate-950 tracking-tighter italic">{report.participantCount || "0"}</h3>
           </div>
           <div className="absolute -right-10 -bottom-10 h-32 w-32 rounded-full bg-corporate-orange/5 blur-2xl" />
        </div>

        <div className="rounded-[3rem] bg-white border border-slate-100 p-8 shadow-2xl shadow-slate-200/50 flex flex-col justify-center relative overflow-hidden group">
           <div className="relative z-10">
              <div className="h-14 w-14 rounded-2xl bg-blue-50 flex items-center justify-center text-corporate-blue mb-6 border border-blue-100 shadow-sm">
                 <ImageIcon className="h-7 w-7" />
              </div>
              <p className="t3-label mb-1">Medya & Belge</p>
              <h3 className="text-4xl font-black text-slate-950 tracking-tighter italic">{report.mediaFiles.length + report.documents.length}</h3>
           </div>
           <div className="absolute -right-10 -bottom-10 h-32 w-32 rounded-full bg-corporate-blue/5 blur-2xl" />
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
           ...
           <div className="rounded-[3rem] bg-white border border-slate-50 p-10 md:p-14 shadow-xl shadow-slate-200/20 relative overflow-hidden">
              <div className="h-1.5 w-40 bg-corporate-orange mb-10 rounded-full" />
              <h3 className="text-lg font-black text-slate-950 uppercase tracking-widest mb-8 flex items-center gap-3 italic">
                 <Zap className="h-5 w-5 text-corporate-orange" /> Rapor Özeti ve Bulgular
              </h3>
              <div className="space-y-8">
                 <div className="p-8 rounded-[2rem] bg-slate-50 border-l-8 border-corporate-blue shadow-inner italic text-slate-600 font-medium leading-loose">
                    "{report.summary}"
                 </div>
           ...
           <div className="space-y-6">
              <h3 className="text-2xl font-black text-slate-950 tracking-tight uppercase px-4 italic">Ekli Dosyalar</h3>
              <div className="grid md:grid-cols-2 gap-6">
                 {report.mediaFiles.map((file) => (
                    <div key={file.id} className="group p-5 rounded-[2rem] bg-white border border-slate-50 shadow-lg hover:shadow-2xl transition-all cursor-pointer">
                       <div className="flex items-center gap-4">
                          <div className="h-12 w-12 rounded-xl bg-orange-50 text-corporate-orange flex items-center justify-center border border-orange-100 shadow-sm">
                 ...
                 {report.documents.map((doc) => (
                    <div key={doc.id} className="group p-5 rounded-[2rem] bg-white border border-slate-50 shadow-lg hover:shadow-2xl transition-all cursor-pointer">
                       <div className="flex items-center gap-4">
                          <div className="h-12 w-12 rounded-xl bg-blue-50 text-corporate-blue flex items-center justify-center border border-blue-100 shadow-sm">
        ...
        <div className="lg:col-span-1 space-y-8">
           ...
           <div className={cn(
             ...
             report.status === "APPROVED" ? "bg-emerald-500 shadow-emerald-500/20" : "bg-corporate-orange shadow-corporate-orange/20"
           )}>
              <div className="bg-slate-950 rounded-[2.4rem] p-8 text-white relative overflow-hidden h-full">
                 <div className="flex items-center justify-between mb-8">
                    <div className="h-14 w-14 rounded-2xl bg-white/10 flex items-center justify-center border border-white/10">
                       <ShieldCheck className={cn("h-7 w-7", report.status === "APPROVED" ? "text-emerald-400" : "text-corporate-orange")} />
                    </div>
           ...
           {(report.status === "DRAFT" || report.status === "REVISION_REQUESTED") && (
             <div className="rounded-[2.5rem] bg-white border border-slate-100 p-8 shadow-xl shadow-slate-200/20 space-y-5">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                 <Pencil className="h-3 w-3 text-corporate-blue" /> RAPORU DÜZENLE
               </p>
               <form action={updateReportAction as any} className="space-y-4">
                 <input type="hidden" name="reportId" value={report.id} />
                 <div className="space-y-1.5">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Başlık</label>
                 ...
                 <div className="space-y-1.5">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Özet</label>
                 ...
                 <div className="space-y-1.5">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">İçerik</label>
                 ...
                 <div className="space-y-1.5">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Katılımcı Sayısı</label>
                 ...
                 <button className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-950 text-white text-[10px] font-black uppercase tracking-widest py-3.5 hover:bg-corporate-blue active:scale-95 transition-all">
                   <CheckCircle2 className="h-4 w-4" /> RAPORU GÜNCELLE
                 </button>
               </form>
           ...
           {report.event && (
              <Link href={`/baskan/etkinlikler/${report.event.id}`} className="block group">
                 <div className="rounded-[2.5rem] bg-white border border-slate-100 p-8 shadow-xl shadow-slate-200/20 hover:border-corporate-blue transition-all relative overflow-hidden">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4 italic">// İLGİLİ PROJE/ETKİNLİK</p>
                    <div className="flex items-center justify-between">
                       <div>
                          <h4 className="text-sm font-black text-slate-950 uppercase tracking-tight group-hover:text-corporate-blue transition-colors">{report.event.title}</h4>
                       ...
                       <div className="h-10 w-10 rounded-xl bg-slate-50 flex items-center justify-center text-corporate-blue group-hover:scale-110 transition-transform">
                          <ChevronRight className="h-5 w-5" />
                       </div>
                    </div>
                    <div className="absolute bottom-0 right-0 h-1 w-0 bg-corporate-blue group-hover:w-full transition-all duration-700" />
                 </div>
              </Link>
           )}
           ...
           <Link 
             href="/baskan/raporlar"
             className="w-full h-16 rounded-2xl bg-slate-100 text-slate-950 text-[11px] font-black uppercase tracking-widest border border-slate-200 shadow-sm active:scale-95 transition-all flex items-center justify-center hover:bg-slate-950 hover:text-white"
           >
              RAPORLARA DÖN
           </Link>
        </div>
